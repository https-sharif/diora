...
export const messageService = {
  async getConversations(token: string): Promise<{ status: boolean; conversations: Conversation[] }> {
    const response = await axios.get(`${config.apiUrl}/api/message/conversations`, {
  ...
  async sendMessage(
    ...
    const response = await axios.post(
      ...
      { conversationId, text, type, replyTo, productId, profileId, postId, imageUrl },
      {
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  async markMessagesAsRead(conversationId: string, token: string): Promise<{ status: boolean; message: string }> {
  ...
  async deleteMessage(messageId: string, token: string): Promise<{ status: boolean; message: string }> {
  ...
  async addUser(conversationId: string, token: string, users: string[]): Promise<{ status: boolean; conversation: Conversation, message: string }> {
    const response = await axios.put(
  ...
  async leaveGroup(conversationId: string, token: string): Promise<{ status: boolean; message: string }> {
    const response = await axios.put(
  ...
  async updateGroupName(conversationId: string, name: string, token: string): Promise<{ status: boolean; conversation: Conversation }> {
    const response = await axios.put(
  ...
  async updateGroup(conversationId: string, data: { name?: string; avatar?: string }, token: string): Promise<{ status: boolean; conversation: Conversation }> {
    const response = await axios.put(
