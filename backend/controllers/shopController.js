...
export const getShopById = async (req, res) => {
  ...
  try {
    ...
    const shop = await Shop.findById(shopId).populate('userId', 'username avatar');
...
export const createShop = async (req, res) => {
  ...
  try {
    const { name, username, description, logoUrl, coverImageUrl, location, contactEmail, contactPhone, website, categories } = req.body;
...
export const updateShop = async (req, res) => {
    ...
    try {
        ...
        if (existingShop.userId.toString() !== req.user.id) {
            return res.status(403).json({ status: false, message: 'Not authorized to update this shop' });
        }

        // Check if username is already taken by another shop
        if (username && username !== existingShop.username) {
            const usernameExists = await Shop.findOne({ username, _id: { $ne: shopId } });
            if (usernameExists) {
                return res.status(400).json({ 
                    status: false, 
                    message: 'Username is already taken' 
...
export const deleteShop = async (req, res) => {
  ...
  try {
    ...
    if (existingShop.userId.toString() !== req.user.id) {
        return res.status(403).json({ status: false, message: 'Not authorized to delete this shop' });
    }
...
export const getTrendingShops = async (req, res) => {
  ...
  try {
    ...
    if (!shops || shops.length === 0) {
      return res.status(404).json({ status: false, message: 'No trending shops found' });
    }
